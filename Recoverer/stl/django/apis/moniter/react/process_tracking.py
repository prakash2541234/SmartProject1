...
def _table_entity_url(ritm_number: str, catalog_task_sysid: str) -> str:
...
def _table_partition_url(ritm_number: str) -> str:
    partition_key = (ritm_number or "").replace("'", "''")
    return f"{_storage_base_url()}/{get_table_name()}?$filter=PartitionKey%20eq%20'{partition_key}'"
...
def _fetch_partition_entities(ritm_number: str) -> list[dict[str, Any]]:
    response = requests.get(
        _table_partition_url(ritm_number),
...
def query_process_status(
    ...
    if catalog_task_sysid:
        response = requests.get(
            _table_entity_url(ritm_number, catalog_task_sysid),
            headers=_table_headers(),
            timeout=20,
        )
    ...
    candidates = []
    for entity in _fetch_partition_entities(ritm_number):
