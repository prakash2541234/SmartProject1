...
def mask_secret(value, visible_chars=4):
    """Return a masked preview of a secret value for safe debugging."""
    if value is None:
        return None
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Check if the network is standalone or on-prem extension
        if req_body["standalone"]:
            # Load DigiCert secrets from the centralized vault for standalone networks
            digicert_details = get_digicert_details(credential)
        else:
            # Skip fetching DigiCert details for on-prem extension networks
            digicert_details = None
        ...
        response_payload = {
            ...
            "details": {
                ...
                "digicert_details": {
                    key: (value if key == "provider" else mask_secret(value))
                    for key, value in digicert_details.items()
                },
            },
        }
