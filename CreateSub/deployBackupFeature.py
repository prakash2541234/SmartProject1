...
def validate_body(body: Dict) -> Tuple[Optional[Dict], Optional[func.HttpResponse]]:
    ...
    return {
        ...
        "force_recreate_dp_policies": bool(body.get("force_recreate_dp_policies", False)),
    }, None
...
def build_dataprotection_policy_body(
    ...
    if frequency_tuple:
        ...
        if normalized_token != interval_token:
            normalized_interval = schedule_interval.replace(interval_token, normalized_token)
    else:
...
def create_backup_policies(
    ...
    for label, hhmm in POLICY_TIMES:
        ...
        for tier, blob_retention_rules, blob_tier_tagging_criteria in blob_tier_configs:
            ...
            if resource_exists_by_id(client, rid, API_VERSIONS["backup_policy"]):
                if force_recreate_dp_policies:
                    ...
                    logging.info("Skipping existing backup policy (create-only mode): %s", rid)
                    continue
        ...
        for prefix, datasource, default_dur, btype in bv_specs:
            for tier, mysql_pgsql_retention_rules, tier_tagging_criteria in tier_configs:
                ...
                policy_requests.append(
                    (rid, API_VERSIONS["backup_policy"], body)
                )
...
def main(req: func.HttpRequest) -> func.HttpResponse:
        ...
        return json_response(
            {
                ...
                "details": {
                    "Please set up a Log Analytics workspace and provide its ID via the LOG_ANALYTICS_WORKSPACE_ID environment variable.",
                }
                },
            400,
        )
    ...
    try:
        ...
        return json_response(
            {
                "status": "error", 
                "message": "Error during backup infrastructure provisioning: {}".format(str(exc))
            }, 
            500)
        ...
        return json_response(
            {
                "status": "error", 
                "message": "Unhandled error during backup infrastructure provisioning: {}".format(str(exc))
            }, 
            500)